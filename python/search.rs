...
use crate::{DistPair, DistanceData, DistanceSearch, Float, KnnSearch, NdArrayDatasetWithDistance};
...
fn vp_knn_query<N>(
    tree: &VPTree<N>,
    data: ArrayView2<N>,
    queries: ArrayView2<N>,
    distance_fn: &DistanceFn<N>,
    k: usize,
    exclude_self: bool,
) -> Vec<Vec<DistPair<N>>>
where
    N: Float,
{
    ...
    { use rayon::prelude::*; (0..queries.nrows()).into_par_iter().map(do_query).collect() }
...
fn kd_knn_query<N>(
    ...
    data: ArrayView2<N>,
    queries: ArrayView2<N>,
    distance_fn: &KdTreeDistanceFn<N>,
    k: usize,
    exclude_self: bool,
) -> Vec<Vec<DistPair<N>>>
where
...
fn build_cover_tree_f32<'py>(
    ...
    let dataset = NdArrayDatasetWithDistance::with_distance(&array, &*dist_fn);
    let mut rng = Pcg32::seed_from_u64(seed.unwrap_or(0));
    let inner = CoverTree::new_with_sampling(&dataset, 1.3, 0, &mut rng);
