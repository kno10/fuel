...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="OPTICS benchmark using scikit-learn for comparison"
    )
    parser.add_argument("data_path", type=Path, help="Path to CSV or whitespace-separated file")
    parser.add_argument("max_eps", type=float, help="Maximum neighborhood radius")
    parser.add_argument("min_points", type=int, help="Minimum points to form a dense region")
    parser.add_argument("xi", type=float, help="Steepness threshold for Xi extraction (0, 1)")
