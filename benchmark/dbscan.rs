...
use hacs::{MatrixDataAccess, NOISE, VPTree, dbscan};
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    let csv_path = args.next().ok_or(
        "usage: cargo run --features benchmark --bin dbscan_benchmark -- <csv_path> <eps> <min_points>",
    )?;
    ...
    let eps: f64 = args
        ...
        .ok_or("missing eps")?
        .parse()
        .map_err(|_| "eps must be a non-negative number")?;
    ...
    let labels = dbscan(&tree, &data, eps, min_points);
