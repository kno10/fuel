...
use fuel::outlier::{KnnOutlierScore, knn_outlier_scores, odin_outlier_scores};
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    let csv_path = args
        .next()
        .ok_or("usage: cargo run --features benchmark --bin knn_benchmark -- <csv_path> <k>")?;
    ...
    // optional mode: "knn" (default) or "odin"
    let mode = args.next().unwrap_or_else(|| "knn".to_string());
    ...
    let scores = match mode.as_str() {
        "knn" => knn_outlier_scores(&tree, &data, k),
        "odin" => odin_outlier_scores(&tree, &data, k)
            .into_iter()
            .map(|s| KnnOutlierScore {
                index: s.index,
                score: s.score,
            })
            .collect(),
