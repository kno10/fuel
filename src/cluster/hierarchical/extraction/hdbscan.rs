...
use crate::cluster::hierarchical::{HdbscanHierarchy, Merge};
...
#[derive(Debug, Clone, PartialEq)]
pub struct ExtractedHierarchy {
    pub nodes: Vec<HierarchyNode>,
    pub roots: Vec<usize>,
}

impl ExtractedHierarchy {
    #[must_use]
    pub fn new() -> Self {
        Self {
            nodes: Vec::new(),
    ...
    fn push_node(&mut self, distance: f64, members: Vec<usize>, children: Vec<usize>) -> usize {
...
impl Default for ExtractedHierarchy {
    fn default() -> Self {
        Self::new()
    }
}
...
#[derive(Debug, Clone, PartialEq)]
pub struct HdbscanHierarchyExtractionResult {
    pub hierarchy: ExtractedHierarchy,
...
#[must_use]
pub fn extract_clusters_with_noise(
    history: &[Merge<f64>],
    num_clusters: usize,
    ...
    for (i, merge) in history.iter().enumerate() {
        ...
        cur_good += (sa >= min_cluster_size) as isize * -1
            + (sb >= min_cluster_size) as isize * -1
...
#[must_use]
pub fn extract_simplified_hierarchy(
    history: &[Merge<f64>],
    ...
    let mut cluster_map: BTreeMap<usize, SimplifiedTempCluster> = BTreeMap::new();
    ...
    for (i, merge) in history.iter().enumerate() {
        ...
        let a_is_core = core_distances.map_or(true, |core| a >= n || dist >= core[a]);
        let a_not_spurious = aclus
            .as_ref()
            .is_some_and(|c| c.is_not_spurious(min_cluster_size))
            || (aclus.is_none() && min_cluster_size <= 1 && a_is_core);

        let mut bclus = cluster_map.remove(&b);
        let b_is_core = core_distances.map_or(true, |core| b >= n || dist <= core[b]);
        let b_not_spurious = bclus
            .as_ref()
            .is_some_and(|c| c.is_not_spurious(min_cluster_size))
...
#[must_use]
pub fn extract_simplified_hierarchy_hdbscan(
    hierarchy: &HdbscanHierarchy,
    min_cluster_size: usize,
) -> ExtractedHierarchy {
...
#[must_use]
pub fn extract_hdbscan_hierarchy(
    history: &[Merge<f64>],
    core_distances: Option<&[f64]>,
    min_cluster_size: usize,
    hierarchical: bool,
) -> HdbscanHierarchyExtractionResult {
    assert!(min_cluster_size > 0, "min_cluster_size must be positive");
    ...
    let mut cluster_map: BTreeMap<usize, HdbscanTempCluster> = BTreeMap::new();
...
#[must_use]
pub fn extract_hdbscan_hierarchy_hdbscan(
    hierarchy: &HdbscanHierarchy,
    min_cluster_size: usize,
    hierarchical: bool,
) -> HdbscanHierarchyExtractionResult {
...
#[inline]
fn cluster_size(history: &[Merge<f64>], n: usize, id: usize) -> usize {
...
#[derive(Debug, Clone)]
struct SimplifiedTempCluster {
    newids: Vec<usize>,
    depth: f64,
    children: Vec<usize>,
}

impl SimplifiedTempCluster {
    fn new(depth: f64) -> Self {
        Self {
            newids: Vec::new(),
            depth,
            children: Vec::new(),
...
fn simplified_add_id(
    clus: &mut SimplifiedTempCluster,
    id: usize,
    dist: f64,
    as_cluster: bool,
    hierarchy: &mut ExtractedHierarchy,
    history: &[Merge<f64>],
    n: usize,
...
fn simplified_to_node(
    temp: &mut SimplifiedTempCluster,
    hierarchy: &mut ExtractedHierarchy,
) -> usize {
...
#[derive(Debug, Clone)]
struct HdbscanTempCluster {
    members: Vec<usize>,
    dist: f64,
    dmin: f64,
    aggregate: f64,
    children_total: usize,
    children: Vec<HdbscanTempCluster>,
}

impl HdbscanTempCluster {
    fn new_leaf(dist: f64, point: usize) -> Self {
        Self {
            members: vec![point],
            dist,
            dmin: dist,
    ...
    fn new_parent(dist: f64, a: Self, b: Self) -> Self {
        let children_total = a.total_elements() + b.total_elements();
        Self {
            members: Vec::new(),
            dist,
            dmin: a.dmin.min(b.dmin),
            aggregate: children_total as f64 / dist,
            children_total,
            children: vec![a, b],
        }
    }

    fn grow_cluster(mut self, dist: f64, other: Self) -> Self {
        debug_assert!(other.children.is_empty());
        self.dist = dist;
        self.dmin = self.dmin.min(other.dmin);
        self.aggregate += other.members.len() as f64 / dist;
        self.members.extend(other.members);
        self
    }

    fn grow_point(mut self, dist: f64, point: usize) -> Self {
        self.dist = dist;
        self.dmin = dist;
        self.aggregate += 1.0 / dist;
        self.members.push(point);
        self
    }

    fn reset_aggregate(mut self) -> Self {
        self.aggregate = self.total_elements() as f64 / self.dist;
        self.dmin = self.dist;
        self
    }

    fn total_elements(&self) -> usize {
        self.children_total + self.members.len()
    }

    fn excess_of_mass(&self) -> f64 {
        self.aggregate - self.total_elements() as f64 / self.dist
    }

    fn total_stability(&self) -> f64 {
        let stability = self.excess_of_mass();
        let cstab = self
            .children
            .iter()
            .map(HdbscanTempCluster::total_stability)
            .map(f64::abs)
            .sum::<f64>();
...
#[inline]
fn hdbscan_is_spurious(
    clus: Option<&HdbscanTempCluster>,
    min_cluster_size: usize,
    is_core: bool,
) -> bool {
    match clus {
...
fn finalize_hdbscan_cluster(
    mut temp: HdbscanTempCluster,
    out: &mut HdbscanHierarchyExtractionResult,
    core_distances: Option<&[f64]>,
    parent: Option<usize>,
    flatten: bool,
    hierarchical: bool,
    ...
    let node_id = out
        .hierarchy
        .push_node(temp.dist, std::mem::take(&mut temp.members), Vec::new());
    ...
    for &point in &out.hierarchy.nodes[node_id].members {
        ...
        out.glosh[point] = if cdist > 0.0 { 1.0 - dmin / cdist } else { 0.0 };
    }

    dmin
}

fn collect_hdbscan_children(
    cur: HdbscanTempCluster,
    out: &mut HdbscanHierarchyExtractionResult,
    core_distances: Option<&[f64]>,
    parent: usize,
    flatten: bool,
...
mod tests {
    ...
    fn all_members(extracted_roots: &[usize], nodes: &[super::HierarchyNode]) -> BTreeSet<usize> {
        fn visit(node: usize, nodes: &[super::HierarchyNode], out: &mut BTreeSet<usize>) {
    ...
    fn simplified_hierarchy_wrapper_matches_direct_call() {
        ...
        let hdb = HdbscanHierarchy {
            merges: history.clone(),
            core_distances: core.clone(),
        };
    ...
    fn hdbscan_extraction_returns_glosh_for_all_points() {
        ...
        let extracted = extract_hdbscan_hierarchy(&history, Some(&core), 2, false);
