...
/// Perform NN-Chain clustering in linear memory on vector data.
///
...
#[must_use]
pub fn linear_memory_nn_chain<F: Float, L: GeometricLinkage<F> + Copy>(
    vectors: &[Vec<F>],
    linkage: L,
    is_squared: bool,
) -> MergeHistory<F> {
...
mod tests {
    ...
    use crate::cluster::hierarchical::linkage::CentroidLinkage;

    use super::linear_memory_nn_chain;
    ...
    #[test]
    fn linear_memory_nn_chain_matches_agnes_for_centroid_linkage() {
        let points = vec![
            vec![0.0, 0.0],
            vec![0.2, 0.1],
            vec![3.0, 3.0],
        ...
        let b = linear_memory_nn_chain(&points, CentroidLinkage, true);
