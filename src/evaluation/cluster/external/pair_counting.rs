...
impl PairCounting {
    pub fn new(table: &ClusterContingencyTable) -> Self {
        ...
        for i in 0..table.size1 {
            ...
            if table.break_noise_clusters && table.noise1[i] {
                ...
                in1 += size * if table.self_pairing { size } else { size.saturating_sub(1) };
            }
        }
        ...
        for j in 0..table.size2 {
            ...
            if table.break_noise_clusters && table.noise2[j] {
                ...
                in2 += size * if table.self_pairing { size } else { size.saturating_sub(1) };
            }
        }
        ...
        for i in 0..table.size1 {
            for j in 0..table.size2 {
                ...
                if table.break_noise_clusters && (table.noise1[i] || table.noise2[j]) {
                    ...
                    in_b += size * if table.self_pairing { size } else { size.saturating_sub(1) };
                }
            }
        }

        let tsize = table.contingency[table.size1][table.size2] as u64;
        let total = tsize * if table.self_pairing { tsize } else { tsize.saturating_sub(1) };
        ...
        Self { in_both: in_b, in_first, in_second, in_none }
    }
    ...
    pub fn precision(&self) -> f64 {
        ...
        if d == 0.0 { 0.0 } else { self.in_both as f64 / d }
    }
    ...
    pub fn fowlkes_mallows(&self) -> f64 { (self.precision() * self.recall()).sqrt() }

    pub fn rand_index(&self) -> f64 {
        let d = (self.in_both + self.in_first + self.in_second + self.in_none) as f64;
        if d == 0.0 { 0.0 } else { (self.in_both + self.in_none) as f64 / d }
    }
    ...
    pub fn adjusted_rand_index(&self) -> f64 {
        ...
        let denom = opt - exp;
        if denom == 0.0 { 0.0 } else { (self.in_both as f64 - exp) / denom }
    }
    ...
    pub fn mirkin(&self) -> u64 { self.in_first + self.in_second }
}
