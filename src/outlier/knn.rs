...
///
/// Panics if `k == 0`.
pub fn k_nearest_neighbors_outlier<'a, S, D, F>(
    ...
    S: KnnSearch<F, D::Query<'a>> + Sync,
{
    ...
    Ok(make_outlier_result(scores, "kNN Outlier Score", false, F::zero(), F::zero(), F::infinity()))
}
...
mod tests {
    ...
    fn knn_outlier_ranks_remote_point_highest_vp() {
        ...
        let results = k_nearest_neighbors_outlier(&tree, &data, 2).unwrap();
    ...
    fn knn_matches_reference_outlier_score() {
        ...
        let result = k_nearest_neighbors_outlier(&tree, &data, 10).unwrap();
