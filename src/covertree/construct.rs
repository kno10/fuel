    ...
    fn bulk_construct<D: DistanceData<F>>(
        ...
        if Self::BEST_CENTER_REASSIGN {
            ...
            for i in 0..(bucket_count - 1) {
                let bucket_elems = &mut buckets[i].1;
                for j in (0..bucket_elems.len()).rev() {
                    let point = bucket_elems[j];
                    ...
                    for k in 0..bucket_count {
                        ...
                        let other_center_idx = center_indices[k];
                    ...
                    if best_center != i {
                        let _ = bucket_elems.swap_remove(j);
