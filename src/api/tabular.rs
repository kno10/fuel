    ...
    F: Float,
{
    fn query_distance(&self, b: usize) -> F {
        debug_assert!(b < self.data.len());
        match &self.coords {
            Some(coords) => {
                self.data.distance_fn.distance(coords.as_ref(), self.data.data[b].as_ref())
            }
            None => self.data.distance(self.index, b),
        }
    }
}
