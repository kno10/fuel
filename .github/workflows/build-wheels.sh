...
# For docker builds
dnf update || apt update || apk update || true
dnf -y install openblas-devel || apt install libopenblas-dev || apk add openblas-dev || true
...
# build wheels
for PYBIN in /opt/python/cp3[89]*/bin /opt/python/cp31[012345]-cp31[012345]/bin; do
    "${PYBIN}/pip" install -r requirements.txt -r requirements-dev.txt
    "${PYBIN}/maturin" build -i "${PYBIN}/python" --release --strip --features python,parallel,openblas-system --compatibility linux
done
